# It5:  Prediction using model built on 1871-2016, using blocks of meilaglvl assumptions. and data includes an ensemble of assumptions about band of meilaglvl

# Dec 11 remark on issue of Friday, when I used data block, which has real values of meilaglvl, the result is an omniscient forecast.
# and question was why.  Today I saw why:  It5 adds a column of meilaglvl, but so then, it has two,
# and when it later selects meilaglvl to save, it keeps the first one, the true one, not the synthetic one.
# leading to perfect forecast because it is using persistence..

# The statistical model is made using blocks 1 and 2.
# Note that ao today datablock5 is having some trouble about formatting
# a matrix with the new columns of factors - this is because I added apsides, 
# and because the future segment is not correctly initialized with this column as a factor (I think this is the reason) it is having
# the future block does not work as expected.  So for todayś quick look I am dropping the future block 



inputblock<-data_block7_df
...
# The model is to applied to data in future, so it is necessary to assemble a block of MEI assumptions dimensioned for block5.
...
# It is having a problem with the below, and something likely that is about values
# which look numeric but are actually character.
# The problem begins with line 3205, which is where "future" begins in datablock5.
# It is something about the data appended there.  For now, I will restrict the model
# to 1:3204, though that will not be forecast!
# Note, it still does not work.  This may be something I fixed in the *.pres.  I will go there.
#
# To run ahead I have omitted future from the forecast.  this is causing a mismatch
# with data_block5_df which is 3361 in length, not 3201 without future
#
...
#  mei_test_predict_df <- data.frame(predict(mei_model,testb_n[,-6]))
...
p15+xlim(test_forecast_ensemble$meidate[1500],test_forecast_ensemble$meidate[1750])


p15+xlim(test_forecast_ensemble$meidate[1500],test_forecast_ensemble$meidate[1760])
ggsave("forecast1.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[1750],test_forecast_ensemble$meidate[2010])
ggsave("forecast2.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[2000],test_forecast_ensemble$meidate[2260])
ggsave("forecast3.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[2250],test_forecast_ensemble$meidate[2510])
ggsave("forecast4.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[2500],test_forecast_ensemble$meidate[2760])
ggsave("forecast5.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[2750],test_forecast_ensemble$meidate[3010])
ggsave("forecast6.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[3000],test_forecast_ensemble$meidate[3260])
ggsave("forecast7.png",width=8,height=4,units=c("in"))

p15+xlim(test_forecast_ensemble$meidate[3150],test_forecast_ensemble$meidate[3250])
ggsave("forecast8.png",width=8,height=4,units=c("in"))

pfuture <- p15+xlim(test_forecast_ensemble$meidate[3175],test_forecast_ensemble$meidate[3350])
pfuture + geom_point(data=newmei,aes(edate,MEI),shape=25)

ggsave("thecominglanina.png",width=8,height=4,units=c("in"))


# reference on ggsave: http://ggplot2.tidyverse.org/reference/ggsave.html
